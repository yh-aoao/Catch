...
# Use Leap Hand
XML_DCMM_LEAP_OBJECT_PATH = "urdf/x1_ur5e_leap_right_object.xml"
XML_DCMM_LEAP_UNSEEN_OBJECT_PATH = "urdf/x1_ur5e_leap_right_object.xml"
XML_ARM_PATH = "urdf/ur5e_right.xml"
## Weight Saved Path
...
arm_joints = np.array([
   -1.5021, -1.8165, 1.8391, -2.2738, -1.4934, -0.0100
])
...
# 训练时限制物体形状。设为 ["sphere"] 则只用小球训练，让灵巧手专注学习抓球。
# 设为 None 则使用全部形状 ["box", "cylinder", "sphere", "ellipsoid", "capsule"]。
...
Kp_arm = np.array([300.0, 2000.0, 1000.0, 150.0, 200.0, 50.0])
Ki_arm = np.array([1e-2, 5e-1, 2e-1, 3e-2, 1e-2, 2e-2])
Kd_arm = np.array([40.0, 80.0, 60.0, 15.0, 10.0, 5.0])
llim_arm = np.array([-300.0, -300.0, -300.0, -150.0, -150.0, -100.0])
ulim_arm = np.array([300.0, 300.0, 300.0, 150.0, 150.0, 100.0])
