...
from gym_dcmm.utils.util import *
    ...
    def _reset_object(self):
        ...
        if object_body is not None:
            ...
            if geom is not None:
                ...
                # 根据 train_object_filter 过滤可选物体形状（专注小球训练）
                if hasattr(DcmmCfg, 'train_object_filter') and DcmmCfg.train_object_filter is not None:
                    filter_indices = [i for i, s in enumerate(DcmmCfg.object_shape) if s in DcmmCfg.train_object_filter]
                else:
                    filter_indices = [0, 1, 2, 3, 4]
                object_id = np.random.choice(filter_indices)
