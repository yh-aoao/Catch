"""
Implement the Inverse Kinematics (IK) for the UR5e.
...
from gym_dcmm.utils.util import calculate_arm_Te, angle_axis_python
...
class QP(IK):
    ...
    def step(self, model: mujoco.MjModel, data: mujoco.MjData, Tep: np.ndarray, q: np.ndarray, i: int):
        ...
        Te = calculate_arm_Te(data.body("wrist_3_link").xpos, data.body("wrist_3_link").xquat)
        # print("Tep: ", Tep)
        # print("Te: ", Te)
        # exit(1)
        # Calculate the error
        ...
        mujoco.mj_jacBodyCom(model, data, jacp, jacr, model.body("wrist_3_link").id)
...
class LM_Chan(IK):
    ...
    def step(self, model: mujoco.MjModel, data: mujoco.MjData, Tep: np.ndarray, q: np.ndarray):
        ...
        mujoco.mj_fwdPosition(model, data)
        Te = np.eye(4)
        Te[:3,3] = data.body("wrist_3_link").xpos
        res = np.zeros(9)
        mujoco.mju_quat2Mat(res, data.body("wrist_3_link").xquat)
