...
当前篮筐中心 y=2.2m、站位距离=1.2m，因此目标 y=1.0m；x 随篮筐中心变化。
圆框每回合在世界 X 轴 `[-0.8, 0.8]m` 内均匀采样，Y=2.2m、Z=0.9m 固定。
相比之前的 X 范围 ±0.3m 和站位距离 0.6m，现在左右变化更大、停车位置离框更远。
随机采样仍可能连续抽到邻近位置，并不保证每两个回合都相差很远。
配置在 `configs/env/DcmmCfg.py` 的 `basket_center_x_range` 和 `basket_base_front_dist`，
它们是 Python 配置，不是 Hydra 命令行参数。增大后者会让停车位置沿世界 -Y 方向远离框。
这里的目标是 `arm_base`，它与底盘几何中心有安装偏移，不应以车壳中心判断是否到位。
实际出手距离还取决于手臂姿态，1.2m 是新的站位初值，尚未验证为最优抛球距离。
