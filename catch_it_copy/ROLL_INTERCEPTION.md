...
恢复为 0.30m，默认不再根据手部包围球自动下调高度。包围球估计保留为
空间惩罚和诊断；`roll_use_workspace_height=True` 才启用自适应下调。
取消到位后永久锁定，目标始终根据当前轨迹更新，两个阶段共享拦截目标。
grasping 恢复较弱的真实球距离奖励 `3*exp(-50*d²)`，补充最后靠近球的信号。
...
有效接触奖励、Tracking 接触完成、tracking→grasping 切换仍要求球完整离开
桌子前沿且不再接触桌面，但取消整手 3cm 余量的硬门槛。
闭合塑形根据实际距离或有效接触平滑启用，允许离桌前的近距离预成形。
整手余量不足只扣空间奖励，真实碰桌仍失败；原始接触另记为 `roll_raw_touch`。
...
| 参数 | 默认值 | 含义 |
|---|---|---|
| `roll_wait_height` | 0.30m | link6 最大等待世界高度 |
| `roll_min_wait_height` | 0.14m | 等待高度下限 |
| `roll_grasp_clearance` | 0.03m | 整手到桌板的软余量 |
| `roll_use_workspace_height` | False | 是否根据包围球自动降低目标 |
| `roll_catch_precision_weight` | 3.0 | grasping 的球距离奖励权重 |
...
避免新参数与代码不匹配。Basket 本轮也已修改，见 `ROLL_BASKET_REDESIGN_2026-09-14.md`。
