...
class PPO_Track(object):
    ...
    def action2dict(self, actions):
        ...
        if self.env.call('task')[0] == 'Tracking':
            ...
            arm_tensor = actions[:, 2:8] * self.action_track_denorm[1] # 从5维修改到了8维
            hand_tensor = actions[:, 8:] * self.action_track_denorm[2] # 从5维修改到了8维
        else:
            base_tensor = actions[:, :2] * self.action_catch_denorm[0]
            arm_tensor = actions[:, 2:8] * self.action_catch_denorm[1] # 从5维修改到了8维
            hand_tensor = actions[:, 8:] * self.action_catch_denorm[2] # 从5维修改到了8维
        actions_dict = {
            'arm': arm_tensor,
            'base': base_tensor,
            'hand': hand_tensor
