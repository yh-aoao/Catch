            ...
            if self.stage == "tracking":
                ...
                if self.object_motion == "roll":
                    ...
                    try:
                        ...
                        reward_finger_sync = 5.0 * max(0.0, 0.3 - _mcp_var)
                        # 方向一致性惩罚：三指 MCP 符号不一致时扣分
                        reward_finger_dir_penalty = 0.0
                        if any(v > 0.05 for v in _mcp_vals) and any(v < -0.05 for v in _mcp_vals):
                    ...
                    if getattr(DcmmCfg, 'basket_train_hand', False):
                        try:
                            ...
                            if self.object_throw:
                                reward_hand_extra += 3.0 * max(0.0, 1.0 - _mcp)  # 释放后张开
                            _mcv = float(np.var([_hq[0], _hq[4], _hq[8]]))
                            reward_hand_extra += 2.0 * max(0.0, 0.3 - _mcv)      # 三指同步
                            for _j in [[0,2,3], [4,6,7], [8,10,11], [13,14,15]]:
