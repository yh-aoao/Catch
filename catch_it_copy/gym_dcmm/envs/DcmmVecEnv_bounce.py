...
class DcmmVecEnv(gym.Env):
    ...
    def __init__(
        ...
        self.print_bounce_info = True  # 弹跳模式：每个 episode 自动打印物理参数
        motion_alias = {"弹": "bounce", "tan": "bounce", "bounce": "bounce", "roll": "roll", "throw": "throw"}
    ...
    def compute_reward(self, obs, info, ctrl):
        ...
        if self.object_motion in ("roll", "bounce"):
            ...
            # 手掌朝下奖励。Tracking 中这项由机械臂末端姿态动作控制，不是由 hand 关节控制。
            try:
                rot = quaternion_to_rotation_matrix(obs['arm']['ee_quat'])
                hand_z_world = rot[:, 2]
                dot_down = np.clip(np.dot(hand_z_world, np.array([0, 0, -1.0])), -1.0, 1.0)
                palm_alignment = 0.5 * (dot_down + 1.0)
                if self.task == 'Tracking':
                    w_palm = getattr(DcmmCfg, 'roll_w_palm_tracking', 8.0)
                    w_palm_bonus = getattr(DcmmCfg, 'roll_w_palm_tracking_bonus', 3.0)
                else:
                    w_palm = getattr(DcmmCfg, 'roll_w_palm', 0.5)
                    w_palm_bonus = getattr(DcmmCfg, 'roll_w_palm_bonus', 0.3)
                reward_palm_down = w_palm * palm_alignment
                if dot_down >= self.roll_palm_down_cos:
                    reward_palm_down += w_palm_bonus
            except Exception:
                reward_palm_down = 0.0
        ...
        if self.task == "Catching":
            ...
            if self.stage == "tracking":
                ...
                if self.object_motion in ("roll", "bounce"):
                    rewards = reward_pos_component + reward_height + reward_palm_down + reward_ctrl + reward_collision + reward_constraint + self.reward_touch
                else:
    ...
    def _step_mujoco_simulation(self, action_dict):
        ...
        for i in range(self.steps_per_policy):
            ...
            if self.object_motion in ("roll", "bounce"):
                object_contacts_for_task = object_contacts[object_contacts != self.floor_id]
            else:
            ...
            if not self.terminated:
                ...
                if self.object_motion in ("roll", "bounce") and obj_rel is not None:
                    # roll/bounce 模式：球超过机器人或越过前界就视为越界
                ...
                if self.task == "Catching":
                    ...
                    if self.object_motion == "throw":
                        ...
                        # 4) 球在机器人前方时要求手掌朝下（开局有宽限期，给模型时间转动掌心）
                        elif obj_rel is not None and obj_rel[1] > 0.0:
                            palm_grace = getattr(DcmmCfg, 'roll_palm_grace', 0.8)
                            if (self.Dcmm.data.time - self.start_time) >= palm_grace:
                                try:
                                    obs_current = self._get_obs()
                                    rot = quaternion_to_rotation_matrix(obs_current['arm']['ee_quat'])
                                    hand_z_world = rot[:, 2]
                                ...
                                if not palm_down:
                                    self.terminated = True
                                    self.terminated_reason = 'palm_not_down'
                        # 5) 最近 K 个策略步内没有越靠（no-approach）
                        elif d_xy is not None and len(self.xy_dist_history) == self.xy_dist_history.maxlen:
        ...
        if self.task == 'Catching':
            if self.stage == "tracking":
                ...
                if self.object_motion in ("roll", "bounce"):
                    ...
                    obj_contacts = obj_contacts[obj_contacts != self.floor_id]
                ...
                if self.object_motion in ("roll", "bounce"):
                    ...
                    elif contact_on_palm and self.palm_contact_steps > self.roll_catch_wait_steps:
                        self.terminated = True
                        info['success'] = False
