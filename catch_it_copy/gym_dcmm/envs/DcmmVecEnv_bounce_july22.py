...
import argparse
import math
import configs.env.DcmmCfg_bounce_july22 as DcmmCfg  # 58.18 bounce 基线配置（冻结自 2026-07-22 工作区）
...
class DcmmVecEnv(gym.Env):
    ...
    def __init__(
        ...
        self.Dcmm = MJ_DCMM(viewer=viewer, object_name=object_name, object_eval=object_eval,
                            xml_object_path=DcmmCfg.XML_DCMM_LEAP_OBJECT_PATH,
                            xml_unseen_object_path=DcmmCfg.XML_DCMM_LEAP_UNSEEN_OBJECT_PATH,
                            xml_arm_path=DcmmCfg.XML_ARM_PATH)
    ...
    def compute_reward(self, obs, info, ctrl):
        ...
        # 位置项合并：roll 使用 XY + approach；bounce 使用 3D + approach；throw 保持原有基线项
        if self.object_motion == "roll":
        ...
        if self.task == "Catching":
            ...
            if self.stage == "tracking":
                ...
                if self.object_motion == "roll":
                    rewards = reward_pos_component + reward_height + reward_palm_face + reward_finger_dir + reward_ctrl + reward_collision + reward_constraint + self.reward_touch
                elif self.object_motion == "bounce":
            ...
            # 抓取阶段（grasping）
                ...
                if self.object_motion == "roll":
                    rewards = reward_pos_component + reward_ee_precision + reward_height + reward_palm_face + reward_orient + reward_ctrl + reward_collision + reward_constraint \
        ...
        ## 跟踪任务（Tracking）
            ...
            if self.object_motion == "roll":
                rewards = reward_pos_component + reward_height + reward_ctrl + reward_collision + reward_constraint + self.reward_touch
            elif self.object_motion == "bounce":
    ...
    def _step_mujoco_simulation(self, action_dict):
        ...
        if self.task == "Catching" and self.stage == "tracking":
            if self.object_motion == "bounce":
