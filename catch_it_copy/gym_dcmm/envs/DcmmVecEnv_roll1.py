...
# 修改了roll模式的终止条件版本
"""
变更: 新增 roll 模式判定参数与距离历史（K 步）；在仿真步内更新 xy 距离历史并实现 tracking 的早接触/越界/在球后面/最近 K 步无靠近 判定；在 step() 中用 roll 专用条件切换到 grasping；添加 grasping 阶段的掌面接触 + 连续低速 + XY 对齐 判定为抓取成功，长时间接触未控住判定为失败；保留 throw 模式原有行为。
"""
