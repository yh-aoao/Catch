...
def env_key_callback(keycode):
    ...
    if keycode == 262: # 右方向键（AKA: right）
        cmd_lin_x += 1
    if keycode == 52: # 数字键 4（AKA: 4）- 左转
        cmd_ang -= 0.2
    if keycode == 54: # 数字键 6（AKA: 6）- 右转
        cmd_ang += 0.2
...
class DcmmVecEnv(gym.Env):
    ...
    def __init__(
        ...
        self.floor_id = mujoco.mj_name2id(self.Dcmm.model, mujoco.mjtObj.mjOBJ_GEOM, 'floor')  # 地面 ID
        ...
        self.obs_t_dim = self.obs_dim - 12 - 6  # 跟踪任务观测维度（18）：减去机械手 12+ 关节角度 6
        self.act_t_dim = self.act_dim - 12      # 跟踪任务动作维度（6）：减去机械手 12
        self.obs_c_dim = self.obs_dim - 6       # 抓取任务观测维度（30）：减去关节角度 6
        ...
        self.roll_palm_down_cos = getattr(DcmmCfg, 'roll_palm_down_cos', 0.94)
        # 抓取判定（接触后速度低于阈值且连续N步）
    ...
    def reset(self):
        ...
        # ---------- catch 成功/失败判定（roll 模式专用） ----------
        if self.task == 'Catching' and self.stage == 'grasping' and self.object_motion == 'roll':
            # 是否在掌面接触
            obj_contacts = self.contacts.get('object_contacts', np.array([]))
            ...
            # 球速度（相对于基座的近似速度，使用观测中的速度）
            ball_speed = np.linalg.norm(obs['object']['v_lin_3d'])
            if contact_on_palm and ball_speed <= self.roll_catch_v_thresh:
                self.consecutive_low_vel += 1
            else:
                # 非低速或非接触则重置计数
                if not contact_on_palm:
                    self.consecutive_low_vel = 0
                else:
                    self.consecutive_low_vel = 0
            ...
            if self.consecutive_low_vel >= self.roll_catch_N_control and dxy_grasp <= self.roll_tracking_xy_thresh:
                self.terminated = True
                info['success'] = True
                self.terminated_reason = 'catch_success'

            # 如果长时间接触但未能在规定步数内降速，则判定为失败
            if contact_on_palm and self.palm_contact_steps > self.roll_catch_wait_steps and self.consecutive_low_vel < self.roll_catch_N_control:
                self.terminated = True
                info['success'] = False
    ...
    def compute_reward(self, obs, info, ctrl):
        ...
        reward_approach = 0.0
        reward_height = 0.0
        reward_palm_down = 0.0
        if self.object_motion == 'roll':
            try:
                ee_pos = obs['arm']['ee_pos3d']
                obj_pos = obs['object']['pos3d']
                curr_d_xy = np.linalg.norm(ee_pos[0:2] - obj_pos[0:2])
                prev_d_xy = self.xy_dist_history[-2] if len(self.xy_dist_history) >= 2 else curr_d_xy
            except Exception:
            ...
            sigma_xy = getattr(DcmmCfg, 'roll_sigma_xy', 0.05)
            w_approach = getattr(DcmmCfg, 'roll_w_approach', 0.5)

            # 即时 XY 奖励（距离越小越好）
            reward_xy = w_xy * math.exp(- (curr_d_xy / max(1e-6, sigma_xy))**2)
            # 靠近奖励（鼓励最近一步更靠近）
            reward_approach = w_approach * max(0.0, prev_d_xy - curr_d_xy)

            # 高度奖励：鼓励末端位于物体上方某偏移处（如球上方）
            height_offset = getattr(DcmmCfg, 'roll_height_offset', 0.06)
            sigma_h = getattr(DcmmCfg, 'roll_sigma_h', 0.03)
            w_h = getattr(DcmmCfg, 'roll_w_h', 0.4)
            try:
                d_h = abs(ee_pos[2] - (obj_pos[2] + height_offset))
                reward_height = w_h * math.exp(- (d_h / max(1e-6, sigma_h))**2)
            except Exception:
                reward_height = 0.0
            ...
            try:
                ...
                hand_z_world = rot[:, 2]
                dot_down = np.dot(hand_z_world, np.array([0, 0, -1.0]))
                w_palm = getattr(DcmmCfg, 'roll_w_palm', 0.3)
                reward_palm_down = w_palm * max(0.0, dot_down)
            except Exception:
                reward_palm_down = 0.0

        # 位置项合并：roll 使用 XY + approach；throw 保持原有基线项
        if self.object_motion == 'roll':
        ...
        if self.task == "Catching":
            ...
            # 抓取阶段（grasping）
                ...
                # 总奖励（抓取阶段：位置项包含精度奖励 + roll 的高度与朝下项）
                if self.object_motion == 'roll':
                    rewards = reward_pos_component + reward_ee_precision + reward_height + reward_palm_down + reward_orient + reward_ctrl + reward_collision + reward_constraint \
        ...
        ## 跟踪任务（Tracking）
            ...
            hand_z_axis = np.array([0, 0, 1])
            reward_orient = abs(cos_angle_between_vectors(local_velocity_vector, hand_z_axis)) * DcmmCfg.reward_weights["r_orient"]
            ## 总奖励（若为 roll 则替换位置项与加入高度/朝下奖励）
            if self.object_motion == 'roll':
                rewards = reward_pos_component + reward_ee_precision + reward_height + reward_palm_down + reward_orient + reward_ctrl + reward_collision + reward_constraint + self.reward_touch
            else:
    ...
    def _step_mujoco_simulation(self, action_dict):
        ...
        for i in range(self.steps_per_policy):
            ...
            try:
                ...
                self.xy_dist_history.append(d_xy)
            except Exception:
                # 若计算失败，跳过历史更新
                pass
            ...
            if not self.terminated:
                ...
                if self.task == "Catching":
                    ...
                    if self.object_motion == "throw":
                        ...
                        # 滚动模式下的跟踪失败判定（更严格）
                        # 1) 提前接触（任何物体接触都认为是 tracking 失败）
                        any_object_contact = self.contacts['object_contacts'].size != 0
                        if any_object_contact:
                            self.terminated = True
                        ...
                        # 4) 最近 K 步内没有越靠（no-approach）
                        elif len(self.xy_dist_history) == self.xy_dist_history.maxlen:
                            if (self.xy_dist_history[0] - self.xy_dist_history[-1]) <= 0.0:
                                self.terminated = True
    ...
    def step(self, action):
        ...
        if self.task == 'Catching':
            if self.stage == "tracking":
                # roll 模式使用更严格的 tracking-success 判定
                if self.object_motion == 'roll':
                    ee_pos = obs['arm']['ee_pos3d']
                    obj_pos = obs['object']['pos3d']
                    ...
                    try:
                        rot = quaternion_to_rotation_matrix(obs['arm']['ee_quat'])
                        hand_z_world = rot[:, 2]
                        palm_down = np.dot(hand_z_world, np.array([0, 0, -1.0])) > self.roll_palm_down_cos
                    except Exception:
                        palm_down = False
                    no_contact = self.contacts['object_contacts'].size == 0
                    in_front = obj_pos[1] > 0.0
                    if dxy <= self.roll_tracking_xy_thresh and dz <= self.roll_tracking_z_thresh and palm_down and no_contact and in_front:
                        self.stage = "grasping"
                else:
